...
export default function AdminEquipmentPage() {
  ...
  const fetchEquipments = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/admin/equipments?page=${currentPage}&per_page=10${categoryParam}`,
        {
          headers: {
  ...
  const fetchCategories = async () => {
    try {
      const token = getToken();
      const response = await fetch(`${API_URL}/admin/equipment-categories`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingEquipment 
        ? `${API_URL}/admin/equipments/${editingEquipment.id}`
        : `${API_URL}/admin/equipments`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...formData,
          equipment_category_id: parseInt(formData.equipment_category_id)
        })
      });

      if (response.ok) {
        showNotification(
          editingEquipment ? 'Equipment updated successfully!' : 'Equipment created successfully!',
          'success'
        );
        setShowModal(false);
        resetForm();
  ...
  const handleDelete = async (equipmentId) => {
    ...
    try {
      const token = getToken();
      const response = await fetch(`${API_URL}/admin/equipments/${equipmentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleEdit = (equipment) => {
    ...
    setFormData({
      ...
      equipment_category_id: equipment.category_id.toString(),
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">Manage your gym equipment and machines</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredEquipments.map((equipment) => (
              <div key={equipment.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all overflow-hidden">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-semibold text-gray-900 flex-1">{equipment.name}</h3>
                  </div>
                  <p className="text-sm text-gray-600 mb-3 line-clamp-2">{equipment.description}</p>
                  <div className="mb-4">
                    <span className="text-xs bg-gray-100 px-2 py-1 rounded">{equipment.category_name}</span>
                  </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Cardio Machines"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-600 focus:border-transparent"
                  placeholder="e.g., cardio-machines (auto-generated if empty)"
                />
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">Add multiple images for your equipment</p>
                </div>
              </div>
